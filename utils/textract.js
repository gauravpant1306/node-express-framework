exports.createForm = (data, config) => {
    ...
    let ocrMapKeyValue=getKeyValueRelationship(keyMap, valueMap, blockMap, trimChars)
    let ocrMapKeyLine=getKeyValueMapForLine(data.Blocks)
   Object.keys(ocrMapKeyValue).forEach(k => {
     if(ocrMapKeyValue[k]==='')
     {
       delete ocrMapKeyValue[k];
     }
   });

   Object.keys(ocrMapKeyLine).forEach(k => {
    if(ocrMapKeyLine[k]==='')
    {
      delete ocrMapKeyLine[k];
    }
  });
    return {...ocrMapKeyValue,...ocrMapKeyLine}
  }
  ...
  function getKeyValueMapForLine (blocks) {
    const kvs = {}
