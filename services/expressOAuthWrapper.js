...
ExpressOAuthServer.prototype.authenticate = function(options) {
  ...
  return function(req, res, next) {
    ...
    return Promise.bind(that)
      ...
      .tap(function(token) {       
        res.username =token.user.name;
        res.userid =token.user.id;
        next();
      })
      .catch(function(e) {
...
var handleResponse = function(req, res, response) {
  ...
  if (response.status === 302) {
    ...
    sendResponse(res,message,null,status.success,{data:response.body});   
...
var handleError = function(e, req, res, response, next) {
  ...
  if (this.useErrorHandler === true) {
    ...
    if (e instanceof UnauthorizedRequestError) {
      sendResponse(res,e.name,e.name,status.unauthorized,{data:{ error: e.name, error_description: e.message }});   
      //return res.send();
    }

    sendResponse(res,e.name,e.name,status.server_error,{data:{ error: e.name, error_description: e.message }});   
